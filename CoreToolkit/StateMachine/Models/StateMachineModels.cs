...
namespace CoreToolkit.StateMachine.Models
{
    ...
    public class ExecutionContext
    {
        ...
        /// <summary>
        /// 将 Parameters 与 Results 序列化为 JSON 字符串，用于断点续传恢复
        /// </summary>
        public string SerializeResumeData()
        {
        ...
        /// <summary>
        /// 从 JSON 字符串反序列化并恢复 Parameters 与 Results
        /// </summary>
        public void DeserializeResumeData(string json)
        {
            if (string.IsNullOrEmpty(json))
                return;

            var data = JsonConvert.DeserializeObject<ResumeData>(json);
            if (data == null)
                return;

            Parameters = new ConcurrentDictionary<string, object>(data.Parameters ?? new Dictionary<string, object>());
            Results = new ConcurrentDictionary<string, object>(data.Results ?? new Dictionary<string, object>());
        }
