...
### Claude 优化建议（2026-04-26）

> 以下建议按优先级排序，来自对代码库的架构审查。

#### 🔴 高优先级（建议近期处理）

| # | 功能 | 说明 |
|---|------|------|
| H1 | **统一 C# 语言版本** | `CoreToolkit` 用 C# 7.3，其余用 8.0。统一升至 8.0 后开启 `nullable reference types`，编译期规避 NRE |
| H2 | **SafetyEnabled 编译期移除** | `SafeMotionController.SafetyEnabled = false` 可在运行时关闭所有安全检查，产线隐患极大。建议用 `#if DEBUG` 包裹，Release 编译时该属性不存在 |
| H3 | **补齐 CollisionDetector 单元测试** | AABB 边界接触、Margin 计算、大量体积性能、三种体积类型交叉检测 |
| H4 | **补齐 InterlockEngine 单元测试** | 规则优先级排序、规则冲突、动态增删规则 |
| H5 | **补齐 FlowModuleBase 超时/取消/异常路径测试** | `ParallelModule`、`LoopModule`、`ConditionalModule` 的执行路径 |
| H6 | **数据库版本化迁移工具** | `AutoAddMissingColumns` 只能末尾加列。工业设备升级需改类型/删列/重命名，建议 Migration 表 + 命名迁移类 |

#### 🟡 中优先级（建议 1-2 个月）

| # | 功能 | 说明 |
|---|------|------|
| M1 | **状态机模块级快照（Snapshot）** | 长耗时模块（如 `AxisMoveModule`）执行到一半断电时，恢复会重走整个模块。加 `OnProgressChanged` 钩子 + `StateMachineSnapshot` 表，实现更细粒度恢复 |
| M2 | **SafetyMonitor 周期动态调整** | JOG/手动模式 20-50ms，自动模式 100ms。当前固定 100ms 对手动点动偏慢 |
| M3 | **ExecutionContext 序列化类型安全** | `ResumeDataJson` 存的是 `object`，反序列化类型信息易丢失。建议限定可序列化类型白名单，或 `TypeNameHandling` + 类型校验防反序列化攻击 |
| M4 | **日志异步写入** | `LogRepository` 同步写入 SQLite，安全事件高频触发时可能阻塞 100ms 监控线程。建议内存队列 + 后台批量 flush |

#### 🟢 低优先级（长期）

| # | 功能 | 说明 |
|---|------|------|
| L1 | **碰撞检测空间分割** | 体积 > 50 时 O(N²) 性能下降，建议八叉树/均匀网格 |
| L2 | **.NET Framework → .NET 6/8 迁移评估** | 工业设备生命周期长，但 4.7.2 的长期支持会逐步减少。需评估研华/雷赛 SDK 的 .NET Core 兼容性 |
| L3 | **CI/CD 自动化构建** | GitHub Actions Windows runner：`dotnet build` + `dotnet test` |
...
| # | 功能 | 优先级 | 说明 |
|---|------|--------|------|
| T6.1 | ✅ **AGENTS.md 项目规范** | 中 | 已完成：修订并统一协作规范（编码风格、提交规范、KimiCode×Cursor 协作约定，2026-05-01） |
