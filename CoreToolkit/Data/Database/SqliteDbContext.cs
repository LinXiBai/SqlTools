...
namespace CoreToolkit.Data
{
    ...
    public class SqliteDbContext : IDisposable
    {
        ...
        /// <summary>初始化状态机运行记录表</summary>
        private void InitStateMachineRecordsTable()
        {
            string createTableSql = @"
                CREATE TABLE IF NOT EXISTS StateMachineRecords (
                    Id INTEGER PRIMARY KEY AUTOINCREMENT,
            ...
            // 创建索引
            string createIndexSql = @"
                CREATE INDEX IF NOT EXISTS IX_StateMachineRecords_MachineName ON StateMachineRecords(MachineName);
                CREATE INDEX IF NOT EXISTS IX_StateMachineRecords_StartTime ON StateMachineRecords(StartTime);
                CREATE INDEX IF NOT EXISTS IX_StateMachineRecords_Status ON StateMachineRecords(Status);
                CREATE INDEX IF NOT EXISTS IX_StateMachineRecords_ContextId ON StateMachineRecords(ContextId);
            ";
            Execute(createIndexSql);

            // 自动检测并添加新列
            AutoAddMissingColumns<StateMachineRecord>("StateMachineRecords");
        }
