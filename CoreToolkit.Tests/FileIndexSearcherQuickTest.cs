...
{
    /// <summary>
    /// FileIndexSearcher 完整测试（20万文件）
    /// </summary>
    public class FileIndexSearcherQuickTest
    {
        private readonly string _testDirectory;
        private readonly Random _random = new Random();

        public FileIndexSearcherQuickTest(string testDirectory = null)
        {
            _testDirectory = testDirectory ?? Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "TestData", "FileIndexSearch_200k");
        }

        /// <summary>
        /// 运行完整测试
        /// </summary>
        public void RunTest(int fileCount = 200000)
        {
            Console.WriteLine("==============================================");
            ...
            // 检查是否已有数据
            if (Directory.Exists(_testDirectory) && Directory.GetFiles(_testDirectory, "*.txt").Length >= fileCount * 0.9)
            {
                Console.WriteLine($"    检测到已有测试数据，跳过生成步骤。");
                var existingFiles = Directory.GetFiles(_testDirectory, "*.txt");
                Console.WriteLine($"    现有文件数: {existingFiles.Length:N0}");
            }
            else
            {
        ...
        private void GenerateTestFiles(int fileCount)
        {
            ...
            Parallel.For(0, fileCount, new ParallelOptions { MaxDegreeOfParallelism = Environment.ProcessorCount }, i =>
            {
                ...
                // 生成约6KB的内容
                string content = GenerateFileContent(6 * 1024);
            ...
            Console.WriteLine($"\n    生成完成！");
            Console.WriteLine($"    文件数量: {generatedFiles.Length:N0}");
